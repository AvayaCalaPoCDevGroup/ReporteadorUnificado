...
function obtenerdatos($conn, $fechainicio, $fechafin)
  ...
  //Obtenemos la lista del IVR que contengan un UCID y especificamos el Rango de Fechas en nuestra Sentencia Preparada
	$stmt = $conn->prepare("SELECT replace(enddetails, 'Transferred : ', ''), count(UCID) from CDR where CallType = 0 and reasoncode = '10' and (calltimestamp BETWEEN :fechainicio and :fechafin) group by enddetails");
